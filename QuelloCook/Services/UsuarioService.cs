...
public class UsuarioService : IUsuarioService
{
    ...
    public async Task<List<string>> RegistrarUsuario(RegistroVM registro)
    {
        ...
        if(result.Succeeded)
        {
            ...
            await _emailSender.SendEmailAsync(registro.Email, "QuelloCook - criação de conta", GetConfirmEmailHtml(HtmlEncoder.Default.Encode(url)));

            //cria a conta pessoal do usuario
            Usuario usuario = new()
            {
                UsuarioId = userId,
